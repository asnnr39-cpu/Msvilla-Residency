...
// Tracks the last session we wrote to storage, so render() (called after
// every state change, including login/logout in auth.js) can notice when
// state.session changed and persist/clear it accordingly — without auth.js
// needing to call anything itself. See persistSession()/restoreSession()
// in database.js.
let lastPersistedUsername = undefined;
function render(){
  if((state.session ? state.session.username : null) !== lastPersistedUsername){
    lastPersistedUsername = state.session ? state.session.username : null;
    persistSession(state.session, state.session ? state.session.remember : true);
  }
...
/* =========================================================
   ...
   Any member can add a bill (photo + reason + amount) against
   a specific day; tapping a date on the calendar shows that
   day's bills. Only an admin (or whoever added an entry) can
   delete it.
   ========================================================= */
...
function ensureDailyExpensesCalendarStyles(){
  ...
  style.textContent = `
    ...
    .daily-cal-header{ display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }
    .daily-cal-month{ text-align:center; font-size:19px; font-weight:700; letter-spacing:.5px; }
    .daily-cal-month span{ display:block; font-size:11px; color:var(--muted); font-weight:500; letter-spacing:0; margin-top:2px; }
    .daily-cal-nav{ width:36px; height:36px; border:0; border-radius:50%; background:var(--soft); color:var(--ink); font-size:22px; line-height:1; cursor:pointer; }
    .daily-cal-nav:hover{ opacity:.8; }
    .daily-cal-weekdays, .daily-cal-grid{ display:grid; grid-template-columns:repeat(7,1fr); gap:5px; }
    .daily-cal-weekdays{ margin-bottom:5px; }
    .daily-cal-weekdays div{ text-align:center; font-size:10px; font-weight:700; color:var(--muted); padding:5px 0; }
    .daily-cal-day{ min-height:52px; padding:5px 3px; border:1px solid transparent; border-radius:9px; background:var(--soft); color:var(--ink); cursor:pointer; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; font-family:inherit; }
    .daily-cal-day:hover{ border-color:var(--accent); }
    .daily-cal-day.selected{ border:2px solid var(--accent); }
    .daily-cal-day.today .daily-cal-number{ text-decoration:underline; }
    .daily-cal-number{ font-size:14px; font-weight:700; line-height:18px; }
    .daily-cal-amount{ display:block; margin-top:3px; font-size:9px; font-weight:700; color:var(--accent); white-space:nowrap; max-width:100%; overflow:hidden; text-overflow:ellipsis; }
    .daily-cal-empty{ min-height:52px; }
    .expense-card{ background:var(--soft); border-radius:12px; padding:12px; margin-bottom:10px; }
    .expense-card .top{ display:flex; justify-content:space-between; align-items:flex-start; gap:8px; }
    .expense-card .amt{ font-weight:800; font-size:16px; white-space:nowrap; }
    .expense-card .reason{ margin-top:4px; font-size:13px; }
    .expense-card .meta{ margin-top:6px; font-size:11px; color:var(--muted); }
    .expense-card img.bill-photo{ margin-top:8px; max-width:100%; border-radius:8px; display:block; }
    @media(max-width:360px){
      .daily-cal-grid, .daily-cal-weekdays{ gap:3px; }
      .daily-cal-day{ min-height:46px; }
      .daily-cal-number{ font-size:13px; }
      .daily-cal-amount{ font-size:8px; }
    }
  `;
...
function renderDailyExpenses(){
  ...
  // If the selected date falls outside the month being viewed, snap the
  // selection to the 1st of the viewed month.
